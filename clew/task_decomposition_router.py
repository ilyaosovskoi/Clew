...
Today ``clew/auto_router.py``'s :meth:`AutoRouter.route` classifies the
...
from clew.auto_router import AutoRouter, TaskComplexity, ModelTier, get_auto_router
...
_DECOMPOSE_SYSTEM_PROMPT = (
    "You are the task-decomposition agent for the Clew AI coding assistant. "
...
_MERGE_SYSTEM_PROMPT = (
    "You are the synthesis agent for the Clew AI coding assistant. Your job "
...
class TaskDecompositionRouter:
    ...
    def _log_to_activity(
        ...
        *,
        ...
        """
        try:
            from clew.activity_log import get_activity_log, CATEGORY_INFO
    ...
    def _sync_canvas(self, subtasks: List[Subtask]) -> None:
        ...
        """
        try:
            from clew.agent.task_canvas import get_task_canvas, STATUS_PENDING
    ...
    def _update_canvas_node(self, st: Subtask) -> None:
        ...
        try:
            ...
            canvas = get_task_canvas()
            if st.error:
                from clew.agent.task_canvas import STATUS_FAILED
                canvas.update_status(st.id, STATUS_FAILED, note=st.error[:60])
            else:
                from clew.agent.task_canvas import STATUS_DONE
    ...
    def _decompose(
        ...
        *,
        ...
        from clew.providers.base import ProviderMessage
