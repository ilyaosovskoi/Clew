...
code — which is fine for the Clew maintainers but useless for users
...
entries MERGE with earlier ones):
  ...
  2. ``~/.clew/commands.json`` — user-global extra allow + deny +
  ...
  3. ``<project>/.clew/commands.json`` — same format, project-scoped.
...
class CommandPolicy:
    ...
    # (via ~/.clew/commands.json) or an explicit approval can do that.
...
def _trust_store_path() -> Path:
    return _clew_home() / "project_trust.json"
...
def approve_project_policy(project_root: str) -> bool:
    ...
    ``<project_root>/.clew/commands.json`` for this project. Pins the
...
def describe_pending_grants(policy: "CommandPolicy") -> str:
    ...
    if not pg or not policy.has_pending_grants():
        return ""
    parts = [
        f"This project's .clew/commands.json requests capabilities "
    ...
    parts.append(
        "These stay BLOCKED until approved (CLI: `clew-cli approve-project "
        "--workspace <path>`; GUI: Settings → Project Policy → Approve)."
    )
    return "\n".join(parts)
...
def resolve(
    ...
    *,
    ...
    Layers (later layers EXTEND allow / dangerous_flags; DENY always wins):
      ...
      2. ~/.clew/commands.json (user-global) — TRUSTED: applies in full.
      3. <project>/.clew/commands.json (project-scoped) — RESTRICTIONS
