"""
Clew v2.0.2 — Agent Identity & Tool-call Audit (Goal G5).
...
3.  **Identity propagation.** The TUI / GUI bridge creates a root
    ...
    ``ClewBridge`` — no patching of ``AgentRuntime`` is required.
...
def make_root_identity(name: str = "clew-root") -> AgentIdentity:
    """Create the root identity for a Clew process."""
...
# the lifetime of the process. ``ClewBridge`` calls ``get_root_identity()``
