...
- ``clew/activity_log.py`` — the existing ``ActivityLog`` singleton
  (every tool call is already recorded there).
- ``clew/agent/encrypted_prompt.py`` — the existing ChaCha20 key-storage
  pattern. We reuse the same ``~/.clew/`` directory convention and
...
1. A local Ed25519 keypair, generated on first use and stored under
   ``~/.clew/audit_key`` (private) and ``~/.clew/audit_key.pub``
   (public). The private key file is chmod 0600. Zero-cloud — the key
   never leaves the user's machine, exactly like the rest of Clew's
...
# ── Key storage ────────────────────────────────────────────────────────
# We reuse the ~/.clew/ directory convention. Files:
#   ~/.clew/audit_key       — 32-byte Ed25519 private key (raw, chmod 0600)
#   ~/.clew/audit_key.pub   — 32-byte Ed25519 public key (raw, chmod 0644)
...
def _private_key_path() -> Path:
    return _clew_home() / _PRIVATE_KEY_NAME


def _public_key_path() -> Path:
    return _clew_home() / _PUBLIC_KEY_NAME
...
def _save_keypair(priv_bytes: bytes, pub_bytes: bytes) -> None:
    """Persist the keypair to ~/.clew/ with restrictive permissions."""
...
def load_or_create_keypair() -> Tuple[bytes, bytes]:
    """Load the Ed25519 keypair from ~/.clew/, creating it on first use.
...
def export_signed_json(entries: List[Dict[str, Any]]) -> str:
    ...
    on first call (and stored to ~/.clew/ for future use).
