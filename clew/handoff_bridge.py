"""
Clew v2.0.2 — Post-task "bridge" / editable handoff (Goal G6).
...
2.  **HandoffStore** — persists handoff documents to
    ``~/.clew/handoffs/<id>.json``. Each doc has:
      * ``id``          — uuid
...
5.  **No telemetry, no network.** Everything is local files under
    ``~/.clew/handoffs/``. The export is a single JSON or Markdown
...
class HandoffStore:
    """Persists HandoffDocuments to ~/.clew/handoffs/<id>.json."""

    def __init__(self, root: Optional[Path] = None) -> None:
        self._root = root or (Path.home() / ".clew" / "handoffs")
