...
- The same ``~/.clew/config.json`` persistence convention as M1.
...
Configurable via ``~/.clew/config.json`` under the ``consensus`` key,
...
``/second_opinion`` and ``/verify`` pattern — see ``clew_tui/app.py``
and ``clew/web_bridge/bridge.py``.
...
def get_consensus_config() -> ConsensusConfig:
    """Load the consensus config from ``~/.clew/config.json``."""
    cfg = _load_config()
