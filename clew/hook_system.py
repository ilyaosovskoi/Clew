...
Design:
  ...
  - Config persistence via ~/.clew/hooks.json (enabled/disabled per hook, not the
    callback code itself — that lives in user Python modules under ~/.clew/hooks/).
  - User hook modules: any .py file under ~/.clew/hooks/ is auto-loaded on startup.
...
Integration points:
  ...
  - AgentRuntime.run_prompt() calls HookManager.dispatch_user_prompt_submit() before
    the LLM call.
  - ClewBridge (TUI + GUI) exposes register_hook / list_hooks / remove_hook / test_hook.
"""
...
class HookManager:
    ...
    def save_config(self) -> None:
        """Save hook enabled/disabled state to ~/.clew/hooks.json."""
    ...
    def load_user_modules(self) -> int:
        """Auto-load hook modules from ~/.clew/hooks/*.py.
        ...
        for py_file in sorted(hooks_dir.glob("*.py")):
            ...
            try:
                spec = importlib.util.spec_from_file_location(
                    f"clew_user_hook_{py_file.stem}", str(py_file),
                )
