"""
clew.web_bridge.workers — Qt-free threading workers.
...
in :mod:`clew.api_server` runs all generation in plain
...
endpoints on :class:`clew.api_server.ClewAPIServer` directly.
"""
...
class _RemovedQtWorker:
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        raise RuntimeError(
            f"{self._legacy_name} was removed in Clew v2.2.0 — the Qt / "
            "PySide6 GUI is no longer maintained. Use the HTTP API at "
            "`/api/chat/stream` (or `/api/chat/oneshot`) on ClewAPIServer "
            "instead. See clew/api_server.py and clew/web_server.py."
        )
...
class TitleWorker(_RemovedQtWorker):
    ...
    the chat-stream handler in :mod:`clew.api_server`.
    """
