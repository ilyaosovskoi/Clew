"""
Agent Orchestrator — connects Kimi-style modular architecture to Clew's runtime.

CLEW v2 NOTE: This module is preserved as the v1.3 "monkey-patching" bridge.
For new code, prefer `clew.agent.AgentRuntimeV2` which provides a cleaner
...
When `clew.agent.AgentRuntimeV2` is used, this orchestrator is NOT needed.
...
ProgressiveTools, CancelToken) and the existing Clew AgentRuntime.
...
Usage:
  ...
  from clew.agent_orchestrator import patch_runtime
...
def _patch_with_cancel_token(runtime) -> None:
    ...
    from clew.loop.tool_scheduler import CancelToken
...
def _patch_with_tool_scheduler(runtime) -> None:
    ...
    from clew.loop.tool_scheduler import ToolScheduler
...
def _patch_with_subagent_host(runtime) -> None:
    ...
    from clew.session.subagent_host import SubagentHost
    from clew.session.subagent_batch import SubagentBatch
...
def _patch_with_compaction(runtime) -> None:
    ...
    from clew.compaction import CompactionManager
...
def _patch_with_swarm_mode(runtime) -> None:
    ...
    from clew.swarm import SwarmManager
...
def _patch_with_progressive_tools(runtime) -> None:
    ...
    from clew.progressive_tools import (
...
def run_with_new_architecture(runtime, task) -> Any:
    ...
    """
    from clew.agent_runtime import TaskResult, AgentEvent, TaskType
    from clew.loop.turn_loop import TurnLoop, TurnStopReason, ToolCallRequest
    from clew.loop.tool_scheduler import CancelledError
