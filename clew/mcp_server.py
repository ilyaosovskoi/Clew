...
Clew can connect to external MCP servers (as a client), but cannot BE an
MCP server itself.  This module exposes Clew's tools via the MCP protocol
so other agents (Claude Code, Codex, etc.) can call Clew as a tool provider.

Protocol:
  - MCP over stdio using JSON-RPC 2.0 with Content-Length framing.
  - The server is started by the external agent as a subprocess.
  - Command: clew-acp --mcp-server  (or python -m clew.mcp_server)
  - The server advertises Clew's tools via tools/list.
  - The calling agent sends tools/call with {name, arguments}.
  - The server routes to the appropriate ToolEngine method.

Design:
  - MCPServerMode wraps ToolEngine and AgentRuntime to expose Clew's
...
Integration:
  - New entry point: clew-acp --mcp-server
  - Can also be used programmatically: MCPServerMode().start()
  - Config: ~/.clew/mcp_server.json (allowed tools, write mode, etc.)
...
class MCPServerMode:
    """Exposes Clew's tools as an MCP server over stdio.
    ...
    Or via CLI:
        python -m clew.mcp_server --workspace /path/to/project
        clew-acp --mcp-server
    """
...
def main() -> None:
    ...
    import argparse
    parser = argparse.ArgumentParser(description="Clew MCP Server")
