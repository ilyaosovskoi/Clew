/* ===================================================================
   ...
   ``clew.api_extended`` — it never imports TUI code itself.
   ...
   • mcp_server     — Clew-as-MCP-server status + tool list
...
(function(){
  'use strict';
  if (window.__clewToolsPanelsInstalled) return;
  window.__clewToolsPanelsInstalled = true;
  ...
  var TOOL_META = {
    ...
    mcp_server:     {title: 'MCP Server', subtitle: 'Clew-as-MCP-server status + tool list'},
  ...
  function showHandoffEditor(){
    body.innerHTML = '<div class="cap-run"><h3>New handoff</h3>' +
      ...
      '<label class="cap-var-input"><span>Agent output (paste full text)</span><textarea id="handoffOutput" rows="10" placeholder="Paste the agent response here — Clew will parse it into typed blocks."></textarea></label>' +
      '<div class="cap-run-actions">' +
  ...
  RENDERERS.github = async function(root){
    ...
    root.querySelector('#ghTokenBtn').addEventListener('click', function(){
      var t = prompt('GitHub token (will be stored at ~/.clew/github_token):', '');
      if (t === null) return;
  ...
  RENDERERS.audit = async function(root){
    ...
    root.querySelector('#auditSignedBtn').addEventListener('click', async function(){
      ...
      _toast('Signed audit exported — Ed25519 key at ~/.clew/audit_key', 'success');
    });
  };
  ...
  RENDERERS.github_actions = async function(root){
    root.innerHTML = '<div class="tools-section"><div class="cap-vars-title">Generate GitHub Action workflow</div>' +
      '<p class="tools-note">Generate a workflow YAML that runs Clew on PR / push / dispatch events.</p>' +
    ...
    root.querySelector('#ghaGenBtn').addEventListener('click', async function(){
      try {
        ...
        out.querySelector('#ghaDownload').addEventListener('click', function(){
          downloadText('clew-action.yml', r.yaml || r.content || '');
        });
      } catch(e){ _toast(e.message, 'error'); }
    });
  };
  ...
  RENDERERS.mcp_server = async function(root){
    ...
    var html = '<div class="tools-section"><div class="cap-vars-title">MCP server status</div>' +
      ...
      '</div>' +
      '<p class="tools-note">Run <code>clew-acp --mcp-server --workspace /path</code> to expose Clew\'s tools to other MCP-compatible agents.</p></div>';
  ...
  window.__clewToolsRenderers = RENDERERS;
  window.__clewToolMeta = TOOL_META;
