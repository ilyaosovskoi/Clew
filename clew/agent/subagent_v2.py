...
This is structurally cleaner than Clew v1's role-based whitelist (which was
enforced at dispatch time — fragile, easy to bypass).

Definitions can be extended via `.clew/agents/*.md` (Markdown + YAML frontmatter),
...
# ---------------------------------------------------------------------------
# User-defined sub-agents (.clew/agents/*.md)
# ---------------------------------------------------------------------------


def discover_user_subagents(workspace_root: Optional[str] = None) -> List[SubagentDefinition]:
    """Discover user-defined sub-agents in `.clew/agents/*.md`.
    ...
    search_paths = []
    if workspace_root:
        search_paths.append(Path(workspace_root) / ".clew" / "agents")
        search_paths.append(Path(workspace_root) / ".claude" / "agents")
    search_paths.append(Path.home() / ".clew" / "agents")
...
def spawn_subagent(
    ...
    *,
    ...
    Returns:
        SubagentHandle (future-like) — see `clew.session.subagent_host.SubagentHandle`.
