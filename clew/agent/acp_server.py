...
to connect to Clew via the open Agent Client Protocol and drive the agent
over stdio or HTTP.
...
def cli_main() -> int:
    """CLI entry point: `clew-acp` — run the ACP server over stdio.

    Reads CLEW_WORKSPACE env var for the default working directory.
    ...
    logging.basicConfig(
        level=os.environ.get("CLEW_ACP_LOG_LEVEL", "INFO"),
        format="%(asctime)s [%(levelname)s] %(name)s: %(message)s",
        stream=sys.stderr,  # stdout is the ACP wire; logs go to stderr
    )

    workspace = os.environ.get("CLEW_WORKSPACE", os.getcwd())
    ...
    async def runtime_factory(session_id: str, cwd: str):
        ...
        registry.load_from_config(os.path.expanduser("~/.clew/config.json"))
