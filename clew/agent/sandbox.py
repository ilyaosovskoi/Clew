"""Sandbox — thin Python wrapper around `clew_native.sandbox` (Rust) or pure-Python fallback.

The sandbox applies OS-level kernel restrictions (Landlock on Linux, Seatbelt
on macOS) to the entire Clew process at startup. Once applied, restrictions
...
Usage:
    from clew.agent.sandbox import apply_sandbox, current_sandbox_profile
...
def apply_sandbox(
    ...
    Args:
        ...
        extra_readwrite_paths: extra read-write paths (e.g. ~/.clew for memory log).
    ...
    if not NATIVE_AVAILABLE:
        logger.warning(
            ...
            "Build clew_native (maturin develop --manifest-path clew-native/pyo3/Cargo.toml) "
