...
`CLEW_PROMPT_KEY` (a 32-byte base64-encoded key) or via a keyfile.
...
_DEFAULT_KEY_ENV = "CLEW_PROMPT_KEY"
_DEFAULT_KEY_FILE = "~/.clew/prompt_key"

# Magic prefix to identify Clew-encrypted prompt files.
_MAGIC = b"CLWP1"
...
class EncryptedPromptStore:
    ...
    def decrypt(self, blob: bytes) -> str:
        ...
        if not blob.startswith(_MAGIC):
            raise EncryptedPromptError("not a Clew-encrypted prompt (missing magic)")
