"""Circuit breaker — thin wrapper around `clew_native.circuit_breaker` (Rust) or pure-Python fallback.

Used to wrap every provider call (LLM, MCP) so that flaky endpoints don't
hammer the network indefinitely. Replaces Clew v1's heuristic string-matching
`SubagentBatch._is_rate_limit_error` with real sliding-window error-rate
tracking per (provider, model) key.

Usage:
    from clew.agent.circuit_breaker import CircuitBreakerRegistry
...
class _NativeBreakerAdapter:
    """Adapts the Rust `clew_native.circuit_breaker.CircuitBreaker` to our Python API."""
...
class _NativeRegistryAdapter:
    """Adapts the Rust `clew_native.circuit_breaker.CircuitBreakerRegistry`."""
