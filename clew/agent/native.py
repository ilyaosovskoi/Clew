"""Loader for the Rust extension module `clew_native`.

Design:
- If `clew_native` is installed (via maturin build of `clew-native/pyo3`),
...
- If not installed, fall back to pure-Python implementations of the same
  interfaces (in `clew.agent._fallback_*`). Performance will be lower for
...
# Set CLEW_DISABLE_NATIVE=1 to force pure-Python mode (for debugging / CI).
_FORCE_PURE = os.environ.get("CLEW_DISABLE_NATIVE", "") == "1"
...
def _try_load_native() -> Optional[Any]:
    """Attempt to import the `clew_native` extension module.
    ...
    if _FORCE_PURE:
        logger.info("CLEW_DISABLE_NATIVE=1 — using pure-Python fallback for all subsystems")
        return None

    try:
        mod = importlib.import_module("clew_native")
        _NATIVE = mod
        _NATIVE_VERSION = getattr(mod, "__version__", None)
        logger.info(
            "clew_native extension loaded (version=%s) — sandbox/circuit_breaker/interjection/compaction will use Rust",
            _NATIVE_VERSION,
        )
        return mod
    except ImportError as e:
        logger.info(
            "clew_native extension not available (%s); falling back to pure Python. "
            "To enable native acceleration, build with `maturin develop --manifest-path clew-native/pyo3/Cargo.toml`.",
            e,
        )
        return None
        ...
        logger.warning(
            "clew_native extension failed to load (%s); using pure-Python fallback",
            e,
        )
        return None
...
def get_native_module() -> Optional[Any]:
    """Return the `clew_native` module, or None if not available."""
...
# Bind at module load so `from clew.agent.native import NATIVE_AVAILABLE`
...
def get_sandbox():
    """Return the sandbox submodule of clew_native, or the pure-Python fallback."""
