...
Contrast with G17 (``clew/learning_loop.py``) which is *per-repository*
"what went wrong" learnings. Persona lives at ``~/.clew/persona.md``
...
_MAINTENANCE_SYSTEM_PROMPT = (
    "You are the persona-maintenance agent for the Clew AI coding assistant. "
    "Your ONLY job is to update the user's persona profile file (~/.clew/persona.md) "
...
class PersonaMemory:
    """Per-user persona profile, stored at ``~/.clew/persona.md``.
    ...
    def _pick_maintenance_provider(
        ...
        *,
        ...
        if auto_router is None:
            try:
                from clew.auto_router import AutoRouter, TaskComplexity
...
def _default_path() -> Path:
    """Return the default persona path: ``~/.clew/persona.md``.
    ...
    """
    return Path.home() / ".clew" / "persona.md"
