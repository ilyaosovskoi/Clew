...
This module is the single place where Clew reaches the internet. It is
...
1. ``run_web_search(query, num_results)`` — search the web. Routes
   through ``MCPManager`` (reuses the existing ~/.clew/mcp.json config
...
Zero-config path: a default ``~/.clew/mcp.json`` template entry for a
working no-API-key search server is documented in
``.clew/skills/web-research/SKILL.md`` and shipped as
``docs/mcp_search_template.json`` (not force-installed — the user
opts in by copying it to ``~/.clew/mcp.json``).

Zero-telemetry: nothing here phones home to Clew's own servers (it
...
def get_websearch_status() -> Dict[str, Any]:
    ...
    """
    try:
        from clew.mcp_manager import get_mcp_manager
...
def _discover_search_backends() -> List[Tuple[str, str]]:
    ...
    try:
        ...
        cfg_path = Path.home() / ".clew" / "mcp.json"
        if cfg_path.exists():
...
def run_web_search(
    ...
    if not backends:
        _last_status_msg = (
            ...
            "~/.clew/mcp.json — see .clew/skills/web-research/SKILL.md "
...
_DEFAULT_UA = "Mozilla/5.0 (compatible; ClewAgent/2.1; +https://github.com/zai-shop/clew)"
_FETCH_TIMEOUT = 15.0
_MAX_REDIRECTS = 5
