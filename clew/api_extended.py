"""
clew.api_extended — v2.2.1 extended REST API endpoints.

Adds ~70 new endpoints to :class:`clew.api_server.ClewAPIHandler` so the
browser GUI can reach every backend capability that was previously only
exposed through the TUI bridge (``clew_tui.bridge.ClewBridge``).

This module is loaded by :mod:`clew.api_server` at import time (via
:func:`install`) and patches ``do_GET`` / ``do_POST`` / ``do_DELETE`` so
the new ``/api/...`` routes are dispatched alongside the legacy ones.

Why a separate module?
----------------------
``clew/api_server.py`` is already 2200+ lines. Adding the new endpoints
...
* ``/api/mcp_server/*``          — Clew-as-MCP-server status
...
* Auth is enforced centrally by :meth:`ClewAPIHandler._check_auth` — the
...
def _bridge():
    """Return a shared ``clew_tui.bridge.ClewBridge`` instance.
    ...
    if _bridge_inst is None:
        ...
        _bridge_inst = ClewBridge(workspace=None, section="general")
    return _bridge_inst
...
# ════════════════════════════════════════════════════════════════════
#
# These endpoints wrap clew.providers.custom_providers' YAML config so
...
#
# Schema (stored at ~/.clew/providers.yaml):
#   providers:
#     - provider_id: "my_nim"
...
def _cp_load_all() -> list:
    from clew.providers.custom_providers import load_custom_providers_config
    return load_custom_providers_config() or []


def _cp_save_all(providers: list) -> None:
    from clew.providers.custom_providers import get_config_path, _yaml_dump
...
def _custom_providers_add(handler, body=None):
    ...
    Body:
        ...
        api_key:        str  — secret key (stored in ~/.clew/providers.yaml)
    ...
    try:
        ...
        try:
            ...
            from clew.providers.custom_providers import register_custom_providers
...
def _custom_providers_test(handler, body=None):
    ...
    body = body or {}
    try:
        from clew.providers.openai_compat import OpenAICompatProvider
        from clew.providers.base import ProviderConfig, ProviderCapability
...
# ════════════════════════════════════════════════════════════════════
# MCP server mode (Clew-as-MCP-server)
# ════════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════════
# Installer — patches ClewAPIHandler
# ════════════════════════════════════════════════════════════════════

def install() -> None:
    """Monkey-patch :class:`clew.api_server.ClewAPIHandler` so the new
    ...
    Called automatically at the bottom of :mod:`clew.api_server` —
    users never need to invoke this directly.
    """
    from . import api_server as _as

    ClewAPIHandler = _as.ClewAPIHandler
    _orig_do_get = ClewAPIHandler.do_GET
    _orig_do_post = ClewAPIHandler.do_POST
    _orig_do_delete = ClewAPIHandler.do_DELETE
    ...
    ClewAPIHandler.do_GET = do_GET
    ClewAPIHandler.do_POST = do_POST
    ClewAPIHandler.do_DELETE = do_DELETE
    logger.info(
