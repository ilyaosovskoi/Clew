...
user's disk under ``<project>/learnings/`` (or ``~/.clew/learnings/``
...
# (e.g. read-only mount), we fall back to ~/.clew/learnings/<project_hash>/
...
_GLOBAL_FALLBACK_DIR = Path.home() / ".clew" / "learnings"
...
def _project_learnings_dir(project_path: str) -> Path:
    ...
    Falls back to ``~/.clew/learnings/<hash>/`` if the project dir
