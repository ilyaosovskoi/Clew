"""
Clew v2.0.2 — Team Spend Dashboard (Goal M3).

**Problem (from CLAUDE.md):**
    "Aggregate ``token_history.jsonl`` by org. Useful for Enterprise gate."

    Individual users have ``~/.clew/token_history.jsonl`` (per-process
    token usage log). Enterprises need to **aggregate** these across:

      * Multiple users (e.g. the whole "platform-team" group)
      * Multiple machines (each Clew install writes its own jsonl)
      * Multiple sessions / chats within a single Clew install
...
1.  **Org membership.** A user is identified by their
    ``~/.clew/identity.json`` file, which carries:
...
IDENTITY_PATH = Path.home() / ".clew" / "identity.json"
TEAM_BUDGET_PATH = Path.home() / ".clew" / "team_budget.json"
...
class TeamSpendDashboard:
    ...
    def __init__(
        ...
        if sources is None:
            default_path = Path.home() / ".clew" / "token_history.jsonl"
