"""
Clew v2.2.0 — Auto-Updater (Qt-free).
...
v1.1.5-fix (clew_bug_report.md bug #11): previously the default
``repo`` argument was the placeholder string ``"user/clew"`` and
...
``self._repo == "user/clew"`` forever, and ``check_for_updates()``
...
default the real Clew repo and skipping only on falsy repo values.
"""
