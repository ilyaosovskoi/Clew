"""
Clew v2.0.2 — Smart Cost-Aware Provider Routing (Goal M2).

**Problem (from CLAUDE.md):**
    "Build on ``registry.py`` + ``token_tracker.py`` data. Task
    complexity classifier → model tier catalog."

    The existing ``AutoRouter`` (clew/auto_router.py) already classifies
...
4.  **Persistence.** Caps and demotion thresholds live in
    ``~/.clew/cost_router.json``. The file is small and human-editable.
"""
...
_CONFIG_PATH = Path.home() / ".clew" / "cost_router.json"
