...
```bash
git clone https://github.com/clew-ide/clew.git
cd Clew
...
5. **Local-first** — local inference via Ollama or LM Studio is the default. API providers are an opt-in escape hatch. Don't make any feature that *requires* a cloud model.
...
Open a [GitHub Issue](https://github.com/clew-ide/clew/issues) with:
