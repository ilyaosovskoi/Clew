"""
Clew v2.0.0 — Web Bridge (REAL implementation).
...
class ClewBridge(QObject):
    ...
    def get_guardian_level(self) -> Dict[str, Any]:
        """Return the current Guardian safety-review level.

        Returns {ok: bool, level: 'off'|'dangerous_only'|'all'}.
        """
        try:
            agent = self._get_or_create_agent_runtime()
            cfg = getattr(agent.tools, "_guardian_config", None)
            level = getattr(cfg, "level", "off") if cfg else "off"
            return {"ok": True, "level": level or "off"}
        except Exception as e:
            return {"ok": False, "error": str(e), "level": "off"}

    @Slot(str, result='QVariantMap')
    def set_guardian_level(self, level: str) -> Dict[str, Any]:
        """Set the Guardian safety-review level.

        Persists to ~/.clew/config.json and applies to the live agent.
        Returns {ok: bool, level: str} or {ok: False, error: str}.
        """
        valid = {"off", "dangerous_only", "all"}
        if level not in valid:
            return {"ok": False, "error": f"Invalid level: {level}. Valid: {sorted(valid)}"}
        try:
            from .agent.guardian import GuardianConfig
            agent = self._get_or_create_agent_runtime()
            old = getattr(agent.tools, "_guardian_config", None)
            if old is not None:
                agent.tools._guardian_config = GuardianConfig(
                    level=level,
                    provider_id=old.provider_id,
                    model=old.model,
                    use_subagent=old.use_subagent,
                )
            else:
            ...
            try:
                ...
                pass
            self.guardian_level_changed.emit(level)
            return {"ok": True, "level": level}
        except Exception as e:
            return {"ok": False, "error": str(e)}

    # Signal emitted when the Guardian level changes from the UI.
    guardian_level_changed = Signal(str)
        ...
        return {
            ...
            "modes": [
                {"id": "single", "label": "Single agent",
                 "desc": "Run a single agent on the task (no collaboration)."},
                {"id": "reviewer", "label": "Reviewer",
                 "desc": "Implementer + reviewer loop. The reviewer returns APPROVE/REJECT/MODIFY; up to N iterations."},
                {"id": "codegen", "label": "Codegen",
                 "desc": "Planner decomposes the task; N parallel implementers build the parts; results are concatenated."},
                {"id": "pair", "label": "Pair",
                 "desc": "Two pair-programmer agents alternate turns on the same task."},
                {"id": "observer", "label": "Observer",
                 "desc": "One worker + N read-only observers. Warnings are collected without blocking the worker."},
            ],
        }
    ...
    def run_collaboration(self, mode: str, task: str) -> Dict[str, Any]:
        ...
        try:
            from .collaboration import (
                CollaborationOrchestrator, CollaborationMode, get_mode,
            )
            agent = self._get_or_create_agent_runtime()
            orch = CollaborationOrchestrator(agent, swarm=self._swarm_manager)
            try:
    ...
    def get_queue_stats(self) -> Dict[str, Any]:
        ...
        Returns {ok, stats: {provider_id: {pending, in_flight, max_in_flight, cooldown_until, total_submitted, total_retries, total_errors}}}.
        """
        try:
    ...
    @Slot(result='QVariantMap')
    def get_persistence_backend(self) -> Dict[str, Any]:
        """Return the configured chat-persistence backend.

        Returns {ok, backend: 'json'|'sqlite'}.
        """
        try:
            cfg_path = _config_path()
            cfg: dict = {}
            if cfg_path.exists():
                with open(cfg_path, "r", encoding="utf-8") as f:
                    cfg = json.load(f) or {}
            backend = cfg.get("persistence_backend", "json")
            return {"ok": True, "backend": backend}
        except Exception as e:
            return {"ok": False, "error": str(e), "backend": "json"}

    @Slot(str, result='QVariantMap')
    def set_persistence_backend(self, backend: str) -> Dict[str, Any]:
        """Switch chat persistence between 'json' and 'sqlite'.

        Persists the choice to ~/.clew/config.json. Existing chats are not
        migrated — the new backend applies to subsequent chats.
        """
        valid = {"json", "sqlite"}
        if backend not in valid:
            return {"ok": False, "error": f"Invalid backend: {backend}. Valid: {sorted(valid)}"}
        try:
            cfg_path = _config_path()
            cfg_path.parent.mkdir(parents=True, exist_ok=True)
            cfg: dict = {}
    ...
    @Slot(result='QVariantMap')
    def list_sqlite_sessions(self) -> Dict[str, Any]:
        """List sessions stored in the SQLite backend (~/.clew/chats.sqlite3).

        Returns {ok, sessions: [{id, title, message_count, updated_at}]}.
        """
        try:
            from .session.sqlite_persistence import SQLitePersistence
            db_path = _clew_home() / "chats.sqlite3"
    ...
    def get_compaction_stats(self) -> Dict[str, Any]:
        """Return compaction statistics from the most recent compaction pass.

        Returns {ok, stats: {original_fragments, kept_fragments, dropped_fragments, chars_saved} | None}.
        """
        try:
            agent = self._get_or_create_agent_runtime()
            ...
            return {"ok": False, "error": str(e), "stats": None}
    ...
    def get_tool_catalog_state(self) -> Dict[str, Any]:
        ...
        Returns {ok, loaded: [...], available: [...], prompt_chars_saved: N}.
        """
        try:
            from .progressive_tools import TOOL_CATALOG
            agent = self._get_or_create_agent_runtime()
            engine = getattr(agent, "tools", None)
            # The set of tool names currently registered with the engine.
            try:
            ...
            # Rough estimate of prompt chars saved: each catalog entry contributes
            # ~ name (20) + description (60) + schema boilerplate (~200).
            ...
            return {"ok": False, "error": str(e), "loaded": [], "available": []}
