#!/usr/bin/env python3
"""clew_tui/smoke_test.py — TUI smoke tests (v2.0.0).
...
or:
    pytest clew_tui/smoke_test.py -v
...
@test("clew_tui package imports cleanly")
def _t(r):
    import clew_tui
    check(hasattr(clew_tui, "ClewTUIApp"), "ClewTUIApp missing from clew_tui")
    check(hasattr(clew_tui, "ClewBridge"), "ClewBridge missing from clew_tui")


@test("clew_tui.app exposes ClewTUIApp with bindings")
def _t(r):
    from clew_tui.app import ClewTUIApp
    bindings = ClewTUIApp.BINDINGS
...
@test("clew_tui.bridge exposes ClewBridge and ProviderChoice")
def _t(r):
    from clew_tui.bridge import ClewBridge, ProviderChoice
    check(callable(ClewBridge), "ClewBridge not callable")
    check(hasattr(ProviderChoice, "__dataclass_fields__"),
          "ProviderChoice is not a dataclass")


@test("clew_tui.widgets exports all widget classes including GuardianModal")
def _t(r):
    from clew_tui import widgets
...
@test("ClewBridge exposes v2.0 collaboration API")
def _t(r):
    from clew_tui.bridge import ClewBridge
    for method in ("list_collaboration_modes", "run_collaboration"):
        check(hasattr(ClewBridge, method),
              f"ClewBridge missing {method}")


@test("ClewBridge exposes v2.0 request-queue API")
def _t(r):
    from clew_tui.bridge import ClewBridge
    check(hasattr(ClewBridge, "get_queue_stats"),
          "ClewBridge missing get_queue_stats")


@test("ClewBridge exposes v2.0 persistence-backend API")
def _t(r):
    from clew_tui.bridge import ClewBridge
...
@test("ClewBridge exposes v2.0 context-fragments API")
def _t(r):
    from clew_tui.bridge import ClewBridge
    check(hasattr(ClewBridge, "get_compaction_stats"),
          "ClewBridge missing get_compaction_stats")


@test("ClewBridge exposes v2.0 progressive-tools catalog API")
def _t(r):
    from clew_tui.bridge import ClewBridge
    check(hasattr(ClewBridge, "get_tool_catalog_state"),
          "ClewBridge missing get_tool_catalog_state")


@test("ClewBridge exposes Guardian level API + persistence helpers")
def _t(r):
    from clew_tui.bridge import ClewBridge
...
@test("ClewBridge exposes the v2.0 Guardian handler hook")
def _t(r):
    from clew_tui.bridge import ClewBridge
    check(hasattr(ClewBridge, "set_guardian_handler"),
          "ClewBridge missing set_guardian_handler")
    # The dead `answer_guardian` method should have been removed.
    check(not hasattr(ClewBridge, "answer_guardian"),
          "dead method answer_guardian() should be removed")
    check(hasattr(ClewBridge, "answer_guardian_verdict"),
          "answer_guardian_verdict must remain")


# ── 3. Command palette covers all v2.0 commands ──────────────────────

@test("BUILTIN_COMMANDS includes v2.0 commands")
def _t(r):
...
@test("CommandPalette is a ModalScreen with expected bindings")
def _t(r):
...
@test("CommandPalette uses OptionList.highlighted (not legacy highlight)")
def _t(r):
...
@test("ChatLog exposes add_tool_call with optional sub_label")
def _t(r):
...
@test("ChatLog exposes reviewer-verdict and observer-warnings panels")
def _t(r):
    from clew_tui.widgets.chat_log import ChatLog
...
@test("ChatLog append_token_delta does not drop the first chunk")
def _t(r):
...
@test("StatusBar.update_status accepts guardian parameter")
def _t(r):
...
@test("StatusBar defines GUARDIAN_LABELS for off/dangerous_only/all")
def _t(r):
    from clew_tui.widgets.status_bar import GUARDIAN_LABELS
...
@test("ClewTUIApp._handle_event handles iteration_end")
def _t(r):
    from clew_tui.app import ClewTUIApp
    src = inspect.getsource(ClewTUIApp._handle_event)
    check("iteration_end" in src,
          "_handle_event must handle iteration_end")


@test("ClewTUIApp._handle_event surfaces subagent labels")
def _t(r):
    from clew_tui.app import ClewTUIApp
    src = inspect.getsource(ClewTUIApp._handle_event)
...
@test("ClewTUIApp.on_mount wires set_guardian_handler")
def _t(r):
    from clew_tui.app import ClewTUIApp
    src = inspect.getsource(ClewTUIApp.on_mount)
    check("set_guardian_handler" in src,
          "on_mount must wire set_guardian_handler")


@test("ClewTUIApp.action_toggle_theme calls reload_css")
def _t(r):
    from clew_tui.app import ClewTUIApp
    src = inspect.getsource(ClewTUIApp.action_toggle_theme)
...
@test("ClewTUIApp has _exec_* methods for every v2.0 command")
def _t(r):
    from clew_tui.app import ClewTUIApp
    for method in ("_exec_collab", "_exec_queue", "_exec_storage",
                   "_exec_sessions", "_exec_context", "_exec_tools",
                   "_exec_guardian"):
        check(hasattr(ClewTUIApp, method),
              f"ClewTUIApp missing {method}")


# ── 8. Live bridge smoke (no agent required) ─────────────────────────

@test("ClewBridge.list_collaboration_modes returns 5 modes")
def _t(r):
    from clew_tui.bridge import ClewBridge
    b = ClewBridge(workspace=tempfile.gettempdir())
...
@test("ClewBridge.get_persistence_backend returns 'json' or 'sqlite'")
def _t(r):
    from clew_tui.bridge import ClewBridge
    b = ClewBridge(workspace=tempfile.gettempdir())
...
@test("ClewBridge.get_queue_stats returns a dict (possibly empty)")
def _t(r):
    from clew_tui.bridge import ClewBridge
    b = ClewBridge(workspace=tempfile.gettempdir())
...
@test("ClewBridge.get_compaction_stats returns None without a live agent")
def _t(r):
    from clew_tui.bridge import ClewBridge
    b = ClewBridge(workspace=tempfile.gettempdir())
...
@test("ClewBridge.get_tool_catalog_state returns a dict with keys")
def _t(r):
    from clew_tui.bridge import ClewBridge
    b = ClewBridge(workspace=tempfile.gettempdir())
...
@test("ClewBridge persistence backend switch round-trips")
def _t(r):
    from clew_tui.bridge import ClewBridge
    # Use a fresh temp HOME so we don't pollute the real ~/.clew/config.json.
    tmp_home = tempfile.mkdtemp(prefix="clew_tui_smoke_")
    old_home = os.environ.get("HOME")
    try:
        os.environ["HOME"] = tmp_home
        b = ClewBridge(workspace=tmp_home)
...
@test("styles_dark.tcss and styles_light.tcss exist and are non-empty")
def _t(r):
...
@test("ApprovalModal and GuardianModal are ModalScreens")
def _t(r):
...
@test("GuardianModal returns approve/reject/use_fix")
def _t(r):
    from clew_tui.widgets.approval_modal import GuardianModal
...
def main() -> int:
    print("=" * 70)
    print("clew TUI v2.0.0 — SMOKE TESTS")
    print("=" * 70)
